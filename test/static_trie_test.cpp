...
class static_trie_test : public ::testing::Test
{
...
protected:
    static constexpr auto make_trie()
    {
	return trie::static_trie<int, 4, max_size, node_number>{keys, values};
    }
};

TEST_F(static_trie_test, ContainsTest)
{
    constexpr auto t = make_trie();
...
TEST_F(static_trie_test, GetTest)
{
    constexpr auto t = make_trie();
...
TEST_F(static_trie_test, GetAllTest)
{
    constexpr auto t = make_trie();
